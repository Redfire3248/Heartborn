...
export function openTableMenu(hud, { atTable = false, tab = null } = {}) {
  ...
  const render = () => {
    const tabs = h('div.tabs',
      ...
      h(`button${hud._tableTab === 'bench' ? '.on' : ''}`, { onclick: () => { hud._tableTab = 'bench'; render(); } }, atTable ? 'Workbench' : 'Hand crafting'));
    const title = h('div.table-head', icon('buildings/workshop', 32), h('div', h('h2', atTable ? 'Crafting Table' : 'Crafting'), h('div.faint', atTable ? 'Forge weapons and armour, or make tools and potions' : 'Basics only. Use a Crafting Table for more.')));
  ...
  const forgeTab = () => {
    ...
    const kinds = h('div.forge-kinds',
      ...[['weapon', 'Weapon'], ['armour', 'Armour']].map(([id, name]) => h(`button.btn.sm${hud._forgeKind === id ? '.primary' : ''}`, { onclick: () => { hud._forgeKind = id; render(); } }, name)));
    ...
    const p = forgePreview(mix, hud._forgeKind);
  ...
  // ------------------------------------------------------------ workbench
  const benchTab = () => {
    const list = RECIPES.filter(r => (r.cat === 'tools' || r.cat === 'potions') && (atTable || !needsTable(r)));
