  ...
  const render = () => {
    ...
    m.el.replaceChildren(m.closeBtn,
      ...
      h('div.egg-row', icon('pets/egg_common', 40), h('div', h('b', `Eggs: ${r.eggs}`), h('div.faint', 'Common 45% · Rare 30% · Epic 16% · Legendary 7.5% · Mythic 1.5%')),
        h(`button.btn${r.eggs ? '.primary' : ''}`, { disabled: !r.eggs, onclick: async () => { const p = hatch(g); if (!p) return; const M = await import('./eggHatch.js'); await M.playHatch(hud, p); render(); } }, 'Hatch')),
